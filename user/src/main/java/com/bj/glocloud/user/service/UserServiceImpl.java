package com.bj.glocloud.user.service;
...
import com.bj.glocloud.exception.CommonBizException;
import com.bj.glocloud.enumeration.ExpCodeEnum;
import com.bj.glocloud.commonservicefacade.user.UserService;
import com.bj.glocloud.req.BatchReq;
import com.bj.glocloud.req.user.*;
import com.bj.glocloud.user.dao.UserDAO;
import com.bj.glocloud.utils.KeyGenerator;
...
public class UserServiceImpl implements UserService {
    ...
    @Override
    public List<AddressEntity> findAddresss(String userId) {
        return userDAO.findAddresss(userId);
    }

    @Override
    public String createAddress(AddressCreateReq addressCreateReq, String userId) {
        // 参数校验
        checkParam(addressCreateReq, userId);

        // AddressEntity
        AddressEntity addressEntity = buildAddressEntity(addressCreateReq, userId);

        // 新增
        userDAO.createAddress(addressEntity);
    ...
    public void deleteAddress(String addressId, String userId) {
        // 参数校验
        checkParam(addressId, userId);

        userDAO.deleteAddress(addressId, userId);
    }

    @Override
    public void modifyAddress(AddressUpdateReq addressUpdateReq, String userId) {
        // 参数校验
        checkParam(addressUpdateReq, userId);

        // 修改收货地址
        userDAO.updateAddress(addressUpdateReq, userId);
    }

    private void checkParam(AddressUpdateReq addressUpdateReq, String userId) {
        ...
        if (StringUtils.isEmpty(addressUpdateReq.getLocation())
                && StringUtils.isEmpty(addressUpdateReq.getLocationId())
                && StringUtils.isEmpty(addressUpdateReq.getName())
                && StringUtils.isEmpty(addressUpdateReq.getPhone())
                && StringUtils.isEmpty(addressUpdateReq.getPostCode())) {
            throw new CommonBizException(ExpCodeEnum.LOCATIONUPDATEREQ_NULL);
        }
    }

    private void checkParam(AddressCreateReq addressCreateReq, String userId) {
        // 参数不能为空
        if (addressCreateReq == null) {
        ...
        // 地址不能为空
        if (StringUtils.isEmpty(addressCreateReq.getLocation())) {
            throw new CommonBizException(ExpCodeEnum.LOCATION_NULL);
        }

        // 电话不能为空
        if (StringUtils.isEmpty(addressCreateReq.getPhone())) {
            throw new CommonBizException(ExpCodeEnum.PHONE_NULL);
        }

        // 姓名不能为空
        if (StringUtils.isEmpty(addressCreateReq.getName())) {
    ...
    /**
     * 构建用于新增address的AddressEntity
     *
     * @param addressCreateReq 收货地址创建请求
     * @param userId            用户ID
     * @return
     */
    private AddressEntity buildAddressEntity(AddressCreateReq addressCreateReq, String userId) {
        ...
        addressEntity.setLocation(addressCreateReq.getLocation());
        addressEntity.setName(addressCreateReq.getName());
        addressEntity.setPhone(addressCreateReq.getPhone());
        addressEntity.setPostCode(addressCreateReq.getPostCode());
